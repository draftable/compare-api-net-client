    ...
    internal class SerializationUtils
    {
        ...
        public static Comparison DeserializeComparison([NotNull] string jsonComparison)
        {
            try {
                ...
                throw new Comparisons.UnknownResponseException(jsonComparison, "Unable to parse the response as a comparison.", ex);
        ...
        public static List<Comparison> DeserializeAllComparisons([NotNull] string jsonComparisonArray)
        {
            try {
                ...
                throw new Comparisons.UnknownResponseException(jsonComparisonArray, "Unable to parse the response and extract the array of comparison results.", ex);
            } catch (NullReferenceException ex) {
                throw new Comparisons.UnknownResponseException(jsonComparisonArray, "Unable to parse the response and extract the array of comparison results.", ex);
            }
        }
        ...
        public static Export DeserializeExport([NotNull] string jsonExport)
        {
            try {
                ...
                throw new Comparisons.UnknownResponseException(jsonExport, "Unable to parse the response as an export.", ex);
            } catch (NullReferenceException ex) {
                throw new Comparisons.UnknownResponseException(jsonExport, "Unable to parse the response as an export.", ex);
            }
        }
